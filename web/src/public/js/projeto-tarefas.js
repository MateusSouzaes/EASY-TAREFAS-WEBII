...
async function carregar() {
  try {
    const res = await fetch(`${API_BASE_URL}/api/projetos/${projetoId}/tarefas`);
...
async function atualizarStatus(id, novoStatus) {
  try {
    const res = await fetch(`${API_BASE_URL}/api/tarefas/${id}`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
...
async function remover(id) {
  ...
  try {
    const res = await fetch(`${API_BASE_URL}/api/tarefas/` + id, { method: 'DELETE' });
...
async function carregarComentarios(tarefaId) {
  try {
    const res = await fetch(`${API_BASE_URL}/api/tarefas/${tarefaId}/comentarios`);
...
async function adicionarComentario() {
  ...
  try {
    const res = await fetch(`${API_BASE_URL}/api/comentarios`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        texto,
